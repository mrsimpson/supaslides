...
import type {CreatePresentationEvent, Presentation, PresentationEvent, PresentationPeek} from 'src/api/types/entities'
...
export const useAudienceStore = defineStore('audienceStore', {
    ...
    actions: {
        ...
        async join(joinCode: Presentation['join_code']) {
            ...
            const presentationPeek = await backend.joinPresentation(joinCode, displayName,
                session?.user.id,
                anonUuid)
